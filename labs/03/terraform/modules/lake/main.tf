# =============================================================================
# Module "lake" — the six data-lake resources, moved verbatim from the flat
# root stack. Logical names and arguments are intentionally unchanged: only the
# state address changes (aws_x.y -> module.lake.aws_x.y), so nothing is
# recreated.
# =============================================================================
...
resource "aws_glue_catalog_table" "corridas" {
  ...
  storage_descriptor {
    ...
    # Expanded from single-line blocks: Terraform >= 1.16 rejects a one-line
    # block with more than one argument (same scaffold issue as Lab 02).
    columns {
      name = "corrida_id"
      type = "string"
    }
